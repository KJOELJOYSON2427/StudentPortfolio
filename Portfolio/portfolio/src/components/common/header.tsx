...
import SocialIcons from "../SocialIcons" // 👈 Import your social icons
          ...
          <div className="hidden md:flex items-center gap-8">
            {menuItems.map((item) => (
              <motion.a
                key={item.id}
                href={item.href}
                className="font-semibold transition hover:text-gray-300"
                whileHover={{ scale: 1.05, color: '#66ffcc' }}
                whileTap={{ scale: 0.95 }}
                layout
              >
                {item.label}
        ...
        {isMobileMenuOpen && (
          <>
            ...
            >
              ...
              <div className="flex flex-col py-6">
                {menuItems.map((item, index) => (
                  <motion.a
                    key={item.id}
                    href={item.href}
                    className="px-6 py-4 font-medium text-white hover:bg-gray-800 transition-colors border-b border-gray-800 last:border-b-0"
              ...
              {/* ✅ Replace with your animated SocialIcons */}
              <motion.div
